...
<div class="left-title"><h1>Top Facts</h1></div>
...
// How many adjacent pages should be shown on each side?
	...
	$query = $mysqli->query("SELECT COUNT(*) as num FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 ORDER BY facts.views DESC");
	...
	if($page) 
 	...
 	/* Get data. */
	$result = $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 ORDER BY facts.views DESC LIMIT $start, $limit");
	...
	$q= $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 ORDER BY facts.views DESC LIMIT $start,$limit");

	$numr = mysqli_num_rows($q);
	if ($numr==0)
	{
	echo '<div class="post-box"><div class="msg">There are no posts at the moment. Check back again soon!!</div></div></div>';
	}
