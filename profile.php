...
<div class="left-title"><h1><?php echo $ProfileUsername;?> Profile</h1></div>
...
<div id="profile-image">
 ...
 <?php	if (empty($ProfileImage)){ 
	echo  '<img class="profile-image" src="timthumb.php?src=http://'.$settings['siteurl'].'/templates/'.$settings['template'].'/images/avatar.png&amp;h=200&amp;w=200&amp;q=100" alt="'.$ProfileUsername.'">';
		}else{

	echo  '<img class="profile-image" src="timthumb.php?src=http://'.$settings['siteurl'].'/avatars/'.$ProfileImage.'&amp;h=200&amp;w=200&amp;q=100" alt="'.$ProfileUsername.'"/>';
 }?>
 
<div id="profile-details"> 
<div class="profile-row"><strong>Name:</strong> <?php echo $ProfileUsername;?></div><!--profile-row-->
<div class="profile-row"><strong>Member Since:</strong> <?php echo $ProfileRow['reg_date'];?></div><!--profile-row-->

<div class="profile-row"><strong>Gender:</strong> <?php if(!empty($Gender)){echo $Gender;}else{ echo "Not specified";}?></div><!--profile-row-->

<div class="profile-row"><strong>Birthday:</strong> <?php if(!empty($BirthDay)){ echo $BirthDay;?> (<?php print getAge($BirthDay);?> years old)<?php }else{?>Not specified<?php }?></div><!--profile-row-->
<div class="profile-row"><strong>Country:</strong> <?php if(!empty($Location)){echo $Location;}else{ echo "Not specified";}?></div><!--profile-row-->
</div><!--profile-details-->
</div><!--profile-image-->
...
<div class="posts">
<div class="post-box">
<h1>About <?php echo $ProfileUsername;?></h1>
...
// How many adjacent pages should be shown on each side?
	...
	$query = $mysqli->query("SELECT COUNT(*) as num FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and user_id='$pid' ORDER BY facts.id DESC");
	...
	if($page) 
 	...
 	/* Get data. */
	$result = $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and user_id='$pid' ORDER BY DESC LIMIT $start, $limit");
	...
	$q= $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and user_id='$pid' ORDER BY facts.id DESC LIMIT $start,$limit");

	$numr = mysqli_num_rows($q);
	if ($numr==0)
	{
	echo '<div class="post-box"><div class="msg">User havent posted any posts yet.</div></div></div>';
	}
