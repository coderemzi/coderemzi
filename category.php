...
<div class="left-title"><h1><?php echo $CName;?></h1></div>
...
// How many adjacent pages should be shown on each side?
	...
	$query = $mysqli->query("SELECT COUNT(*) as num FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and facts.catagory_id='$cid' ORDER BY facts.id DESC");
	...
	if($page) 
 	...
 	/* Get data. */
	$result = $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and facts.catagory_id='$cid' ORDER BY facts.id DESC LIMIT $start, $limit");
	...
	{	
		...
		//previous button
		if ($page > 1) 
			$pagination.= "<a href=\"category-$Cid-$CUrl-$prev.html\">« previous</a>";
		else
		...
		{	
			for ($counter = 1; $counter <= $lastpage; $counter++)
			{
				if ($counter == $page)
					...
					$pagination.= "<a href=\"category-$Cid-$CUrl-$counter.html\">$counter</a>";					
			}
		}
		elseif($lastpage > 5 + ($adjacents * 2))	//enough pages to hide some
		{
			...
			if($page < 1 + ($adjacents * 2))		
			{
				...
				$pagination.= "...";
				$pagination.= "<a href=\"category-$Cid-$CUrl-$lpm1.html\">$lpm1</a>";
				$pagination.= "<a href=\"category-$Cid-$CUrl-$lastpage.html\">$lastpage</a>";		
			}
			//in middle; hide some front and some back
			elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2))
			{
				$pagination.= "<a href=\"category-$Cid-$CUrl-1.html\">1</a>";
				$pagination.= "<a href=\"category-$Cid-$CUrl-2.html\">2</a>";
				$pagination.= "...";
		...
		//next button
		if ($page < $counter - 1) 
			$pagination.= "<a href=\"category-$Cid-$CUrl-$next.html\">next »</a>";
		else
			$pagination.= "<span class=\"disabled\">next »</span>";
		$pagination.= "</div>\n";		
	}
	
	$q= $mysqli->query("SELECt * FROM facts LEFT JOIN users ON users.uid=facts.user_id WHERE facts.user_id=users.uid and facts.active=1 and facts.catagory_id='$cid' ORDER BY facts.id DESC LIMIT $start,$limit");

	$numr = mysqli_num_rows($q);
	if ($numr==0)
	{
	echo '<div class="post-box"><div class="msg">There are no posts at the moment. Check back again soon!!</div></div>';
	}
